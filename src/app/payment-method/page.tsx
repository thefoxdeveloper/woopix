"use client";

import PixOne from "@/components/PixOne";
import PixParcelado from "@/components/PixParcelado";
import * as RadioGroup from "@radix-ui/react-radio-group";

import Image from "next/image";
import { useState } from "react";

export default function PaymentMethod() {
  const parcelas = 6;
  const totalValue = 30500;
  const descontoParcela = {
    parcela: 3,
    desconto: 10,
  };
  const [value, setValue] = useState<string | null>(null);
  const cardsData = Array.from({ length: parcelas }, (_, index) => {
    const valuePerInstallment = totalValue / (index + 2);
    return {
      value: valuePerInstallment,
      desconto:
        index + 1 === descontoParcela.parcela
          ? descontoParcela.desconto
          : -1 + -index,
    };
  });
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-8 p-8 ">
      <Image src="/logo.svg" alt="Woovi Logo" width={123} height={36} />

      <span className="text-2xl font-extrabold text-[#4D4D4D] text-center [text-shadow:_0_4px_4px_rgb(0_0_0_/_40%)] font-nunito ">
        João, como você quer pagar?
      </span>
      <RadioGroup.Root
        onValueChange={(value) => setValue(value)}
        className="flex flex-col w-full "
      >
        <div className="mb-8">
          <PixOne value={30500} cashback={3} isSelected={value === "x"} />
        </div>

        {cardsData.map((data, index) => (
          <PixParcelado
            key={index}
            value={data.value}
            desconto={data.desconto}
            index={index}
            isFirst={index === 0}
            isLast={index === cardsData.length - 1}
            isSelected={value === index.toString()}
          />
        ))}
      </RadioGroup.Root>
      <button
        className="cursor-pointer transition-all bg-[#03d69d] text-white px-6 py-2 rounded-lg
       border-[#17a17d]
        border-b-[4px] hover:brightness-110 hover:-translate-y-[1px] hover:border-b-[6px]
        active:border-b-[2px] active:brightness-90 active:translate-y-[2px]"
      >
        Ir para pagamento
      </button>
      <Image src="/pagamento.svg" alt="Woovi Logo" width={250} height={36} />
    </main>
  );
}
